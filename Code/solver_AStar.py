...
def AStarDecide(newNodes, child, existNode, nodeInsts, openList):
    if existNode == 0:   # 0 means 'New Node', which is always kept
        child.updateHeuristic(nodeInsts, 0) # h(n) must be calculated
        ...
        child.updateHeuristic(nodeInsts, existNode) # child's h(n) is equal to existing node's, since they represent the same state
...
while 1:
    if openList == []:
        break
    nodeID = AStarSel(openList) # The node with least evaluation function (g+h) is selected
    ...
    for child in possNodes:
        ...
        AStarDecide(newNodes, child, existNode, nodeInsts, openList) # Processes each possible node to determine if it should be kept or discarded


    latestID     = len(nodeInsts)
    newNodesSize = len(newNodes)
    nodeInsts.extend(newNodes)
    AStarPlace(latestID, newNodesSize, openList) # Places node in openList as a priority queue